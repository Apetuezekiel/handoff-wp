 ...
 * model — reads and writes the same 'zsch_config' option that the
 ...
 * forms:
 *   1. Activate form — posts _zsch_setup_complete=1 → sanitize() in
 *      ZSCH_Admin_Settings sets enabled=true and setup_completed=true.
 *   2. Dismiss form  — posts _zsch_setup_dismiss=1  → sets
...
class ZSCH_Setup_Flow {
	 ...
	 *
	 * Reads $_GET['zsch_step'], sanitize_key, validates against STEPS.
	 * Defaults to 'roles' when absent or invalid.
	 *
	 * @return string
	 */
	public function get_current_step(): string {
		$step = isset( $_GET['zsch_step'] ) ? sanitize_key( $_GET['zsch_step'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display routing; form submission goes through options.php with its own nonce.
	 ...
	 *
	 * Called from ZSCH_Admin_Settings::render_page() when should_show() is true.
	 * The $settings parameter is required so render() can re-use the field
	 * renderers already defined on ZSCH_Admin_Settings without duplicating them.
	 *
	 * @param ZSCH_Admin_Settings $settings
	 */
	public function render( ZSCH_Admin_Settings $settings ): void {
		...
		$next_url = $next
			? admin_url( 'admin.php?page=zicstack-client-handoff&zsch_step=' . $next )
			: admin_url( 'admin.php?page=zicstack-client-handoff' );

		$done_url = admin_url( 'admin.php?page=zicstack-client-handoff' );
		...
		?>
		<div class="wrap zsch-admin-page zsch-setup-flow">
			...
			<div class="zsch-stepper">
				<?php foreach ( self::STEPS as $s ) :
					...
					$css_class = 'zsch-stepper__step';
					if ( $is_done )   { $css_class .= ' zsch-stepper__step--done'; }
					if ( $is_active ) { $css_class .= ' zsch-stepper__step--active'; }
					?>
					<div class="<?php echo esc_attr( $css_class ); ?>">
						<div class="zsch-stepper__dot">
							<?php if ( $is_done ) : ?>
						...
						<span class="zsch-stepper__label"><?php echo esc_html( $s_label ); ?></span>
					</div>
				<?php endforeach; ?>
			</div>
	...
	private function render_settings_step( string $step, string $next_url ): void {
		$page_slug = 'zicstack-client-handoff-' . $step;
		?>
		<div class="zsch-card zsch-setup-form-card">
			...
			<a class="zsch-skip-link" href="<?php echo esc_url( $next_url ); ?>">
	 ...
	 *   1. Activate form: posts _zsch_setup_complete=1 → enables handoff +
	 *      marks setup complete.
	 *   2. Dismiss form: posts _zsch_setup_dismiss=1 → marks setup complete
	...
	private function render_activate_step( string $done_url ): void {
		...
		?>
		<div class="zsch-card">
			<h2 class="zsch-card__title">
				<span class="dashicons dashicons-chart-bar"></span>
				<?php echo esc_html( __( 'Configuration Summary', 'zicstack-client-handoff' ) ); ?>
			</h2>
			<p class="zsch-card__description"><?php echo esc_html( __( 'Review your settings before activating handoff mode.', 'zicstack-client-handoff' ) ); ?></p>

			<!-- Stat grid cards (counts) -->
			<div class="zsch-stat-grid">
				<div class="zsch-stat-card">
					<span class="zsch-stat-card__value"><?php echo esc_html( count( $protected_roles ) ); ?></span>
					<span class="zsch-stat-card__label"><?php echo esc_html( __( 'Protected roles:', 'zicstack-client-handoff' ) ); ?></span>
				</div>
				<div class="zsch-stat-card">
					<span class="zsch-stat-card__value"><?php echo esc_html( count( $admin_roles ) ); ?></span>
					<span class="zsch-stat-card__label"><?php echo esc_html( __( 'Admin roles:', 'zicstack-client-handoff' ) ); ?></span>
				</div>
				<div class="zsch-stat-card">
					<span class="zsch-stat-card__value"><?php echo esc_html( count( $blocked_caps ) ); ?></span>
					<span class="zsch-stat-card__label"><?php echo esc_html( __( 'Blocked capabilities:', 'zicstack-client-handoff' ) ); ?></span>
				</div>
				<div class="zsch-stat-card">
					<span class="zsch-stat-card__value"><?php echo esc_html( count( $protected_plugins ) ); ?></span>
					<span class="zsch-stat-card__label"><?php echo esc_html( __( 'Protected plugins:', 'zicstack-client-handoff' ) ); ?></span>
				</div>
				<div class="zsch-stat-card">
					<span class="zsch-stat-card__value"><?php echo $dashboard_enabled ? '✓' : '—'; ?></span>
					<span class="zsch-stat-card__label"><?php echo esc_html( __( 'Client dashboard:', 'zicstack-client-handoff' ) ); ?></span>
				</div>
			</div>

			<hr class="zsch-card__divider">

			<!-- Activate form -->
			<div class="zsch-activate-actions">
				<form method="post" action="<?php echo esc_url( admin_url( 'options.php' ) ); ?>">
					<?php settings_fields( ZSCH_Core::OPTION_CONFIG ); ?>
					<input type="hidden"
					       name="<?php echo esc_attr( ZSCH_Core::OPTION_CONFIG ); ?>[_zsch_setup_complete]"
					       value="1">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'options.php' ) ); ?>">
					<?php settings_fields( ZSCH_Core::OPTION_CONFIG ); ?>
					<input type="hidden"
					       name="<?php echo esc_attr( ZSCH_Core::OPTION_CONFIG ); ?>[_zsch_setup_dismiss]"
					       value="1">
					<input type="hidden" name="_wp_http_referer" value="<?php echo esc_attr( $done_url ); ?>">
