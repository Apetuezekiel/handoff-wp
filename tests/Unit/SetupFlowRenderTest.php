 ...
 * CH_Setup_Flow::render dispatch tests (B5–B8).
 *
 * WHAT IS TESTED
 *
 * render() is a public dispatcher: it reads the current step (from
 * $_GET['ch_step']), builds the step header, then delegates to either
 ...
 *   B7 — 'activate' step → summary block + _ch_setup_complete hidden input
 *   B8 — 'activate' step → _ch_setup_dismiss hidden input (separate form)
 *
 ...
 * PARAMETER NOTE
 *
 * CH_Setup_Flow::render(CH_Admin_Settings $settings) accepts a settings
 * argument that is currently unused — type-hinted for future extension. A
 * real CH_Admin_Settings instance is passed to satisfy the type hint.
 *
...
class SetupFlowRenderTest extends TestCase {
	...
	/**
	 * Return a CH_Admin_Settings instance wired to the given core.
	 ...
	 *
	 * @param CH_Core $core
	 * @return CH_Admin_Settings
	 */
	private function make_settings( CH_Core $core ): CH_Admin_Settings {
		return new CH_Admin_Settings( $core );
	}
	...
	/**
	 * B5 — render() with no ch_step query param defaults to 'roles' (step 1)
	...
	public function test_render_with_default_step_renders_step_one_of_four_header() {
		...
		$this->assertStringContainsString(
			'Step 1 of 4',
			$html,
			'Header must indicate step 1 of 4 when ch_step is not set (defaults to roles)'
		);
		$this->assertStringContainsString(
			'Configure Roles',
			$html,
	...
	/**
	 * B6 — render() with ch_step=dashboard outputs a header containing
	 ...
	 * and the activate form containing the _ch_setup_complete hidden input.
	 *
	...
	public function test_render_with_activate_step_renders_summary_and_activate_form() {
		...
		$this->assertStringContainsString(
			'name="client_handoff_config[_ch_setup_complete]"',
			$html,
			'Activate form must include the _ch_setup_complete hidden input'
		);
		...
		$this->assertStringContainsString(
			'value="1"',
			$html,
			'The _ch_setup_complete hidden input must have value="1"'
		);
	}
	 ...
	 * containing the _ch_setup_dismiss hidden input.
	 *
	...
	public function test_render_with_activate_step_renders_dismiss_form() {
		...
		$this->assertStringContainsString(
			'name="client_handoff_config[_ch_setup_dismiss]"',
			$html,
			'Activate step must also render the dismiss form with _ch_setup_dismiss input'
		);
		...
		$this->assertStringContainsString(
			'value="1"',
			$html,
			'The _ch_setup_dismiss hidden input must have value="1"'
		);
	}
}
